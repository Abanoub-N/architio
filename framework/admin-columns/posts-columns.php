...
// ADD NEW COLUMN
function mirrorful_columns_head($defaults) {
...
function mirrorful_columns_content($column_name, $post_ID) {
  if ($column_name == 'featured_image') {
    $post_featured_image = mirrorful_get_featured_image($post_ID);
    if ($post_featured_image) {
      echo '<img style="width:100%;" src="' . $post_featured_image . '" />';
    }
  }
}
add_filter('manage_posts_columns', 'mirrorful_columns_head');
add_action('manage_posts_custom_column', 'mirrorful_columns_content', 10, 2);


// ADD NEW COLUMN
function mirrorful_view_head($defaults) {
  $defaults['view_post'] = 'Views';
  return $defaults;
}

// SHOW THE FEATURED IMAGE
function mirrorful_view_content($column_name, $post_ID) {
  global $post;
  $post_id = $post->ID;
  if ($column_name == 'view_post') {
    if(mirrorful_get_post_views($post_id) > 100 ){
      echo "<div class='mirrorful-views' style='color:red;' >";
    }else{
      echo "<div class='mirrorful-views' style='color:green;' >";
    }
    echo '<span class="dashicons dashicons-visibility"></span> ' . mirrorful_get_post_views($post_id) . ' ';
  }
}


add_filter('manage_posts_columns', 'mirrorful_view_head');
add_action('manage_posts_custom_column', 'mirrorful_view_content', 10, 2);

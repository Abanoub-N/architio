...
function mirrorful_pagination( $args = array() ) {
  ...
  if ( $firstpage && (1 != $page) )
  $echo .= '<li class="previous page-item"><a class="page-link" aria-label="Next" href="' . $firstpage . '">' . __( 'First', 'mirrorful' ) . '</a></li>';
  if ( $previous && (1 != $page) )
  $echo .= '<li class="page-item"><a class="page-link" aria-label="Previous" href="' . $previous . '" title="' . __( 'previous', 'mirrorful') . '">' . $args['previous_string'] . '</a></li>';
  ...
  if ($next && ($count != $page) )
  $echo .= '<li class="page-item"><a class="page-link" href="' . $next . '" title="' . __( 'next', 'mirrorful') . '">' . $args['next_string'] . '</a></li>';

  $lastpage = esc_attr( get_pagenum_link($count) );
  if ( $lastpage ) {
    $echo .= '<li class="next page-item"><a class="page-link" href="' . $lastpage . '">' . __( 'Last', 'mirrorful' ) . '</a></li>';
  }
  if ( isset($echo) )

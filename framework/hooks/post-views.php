 ...
 * @param int $postID
 */
function mirrorful_set_post_views($postID) {
    $count_key = 'mirrorful_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    $count_update = '_mirrorful_post_views_count_time';
...
function mirrorful_get_post_views($postID) {
    $count_key = 'mirrorful_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if ($count == '') {
        delete_post_meta($postID, $count_key);
...
function mirrorful_track_post_views($post_id) {
    if (!is_single())
        return;
    if (empty($post_id)) {
        global $post;
    ...
    echo "<!-- mfunc mysecretcode mirrorful_set_post_views($post_id); -->";
    mirrorful_set_post_views(get_the_ID());
    echo "<!-- /mfunc mysecretcode -->";
}

add_action('wp_head', 'mirrorful_track_post_views');

function mirrorful_admin_bar_post_views($wp_admin_bar) {
    ...
    if (is_single()) {
        ...
        $args = array(
            'id' => 'mirrorful-views',
            'title' => '<span class="ab-icon dashicons-visibility"></span> ' . mirrorful_get_post_views($post_id) . ' ' . __('Views', 'mirrorful'),
            'href' => get_edit_post_link($post_id),
            'meta' => array(
                'class' => 'mirrorful-views',
            )
        );
...
add_action('admin_bar_menu', 'mirrorful_admin_bar_post_views', 90);
